...
// Custom icon definitions with SVG paths
const icons = {
  dropdown: (props: SvgIconProps) => (
    <SvgIcon {...props}>
  ...
  selectionControls: {
    checkbox: {
      ...
      indeterminate: (props: SvgIconProps) => (
        <SvgIcon {...props}>
...
// Component configuration by category
const createComponentSettings = (theme: Theme) => ({
  // Layout components
  layout: {
    MuiStack: { useFlexGap: true },
  ...
  formControls: {
    ...
    MuiCheckbox: {
      size: 'small',
      icon: <icons.selectionControls.checkbox.unchecked />,
      checkedIcon: <icons.selectionControls.checkbox.checked />,
      indeterminateIcon: <icons.selectionControls.checkbox.indeterminate />,
    },
    MuiRadio: {
      size: 'small',
      icon: <icons.selectionControls.radio.unchecked />,
      checkedIcon: <icons.selectionControls.radio.checked />,
    },
    MuiFilledInput: {
  ...
  navigation: {
    ...
    MuiTreeView: {
      defaultCollapseIcon: <icons.treeView.collapse />,
      defaultExpandIcon: <icons.treeView.expand />,
      defaultEndIcon: <icons.treeView.end />,
    },
  },
});
...
// Export the flattened default props
export function defaultProps(theme: Theme) {
  const settings = createComponentSettings(theme);

  // Flatten the nested structure into the format MUI expects
  return Object.entries(settings).reduce(
    (acc, [_, categoryProps]) => {
      Object.entries(categoryProps).forEach(([component, props]) => {
        acc[component] = { defaultProps: props };
      });
      return acc;
    },
    {} as Record<string, { defaultProps: any }>
  );
}
