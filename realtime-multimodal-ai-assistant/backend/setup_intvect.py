...
def load_azd_env():
    ...
    result = subprocess.run("azd env list -o json",
...
def setup_index(azure_credential, index_name, azure_search_endpoint, azure_storage_connection_string, azure_storage_container, azure_openai_embedding_endpoint, azure_openai_embedding_deployment, azure_openai_embedding_model, azure_openai_embeddings_dimensions):
    ...
    if index_name in [ds.name for ds in data_source_connections]:
        logger.info(f"Data source connection {
                    index_name} already exists, not re-creating")
    else:
            ...
            SearchIndex(
                ...
                fields=[
                    SearchableField(name="chunk_id", key=True,
                                    analyzer_name="keyword", sortable=True),
                    SimpleField(
                        name="parent_id", type=SearchFieldDataType.String, filterable=True),
                ...
                vector_search=VectorSearch(
                    algorithms=[
                        HnswAlgorithmConfiguration(name="algo", parameters=HnswParameters(
                    ...
                    profiles=[
                        VectorSearchProfile(
                            name="vp", algorithm_configuration_name="algo", vectorizer="openai_vectorizer")
                    ]
                ),
                    ...
                    configurations=[
                        SemanticConfiguration(
                            ...
                            prioritized_fields=SemanticPrioritizedFields(title_field=SemanticField(
                                field_name="title"), content_fields=[SemanticField(field_name="chunk")])
                        )
                    ],
...
if __name__ == "__main__":
    ...
    if os.environ.get("AZURE_SEARCH_REUSE_EXISTING") == "true":
        ...
        logger.info(
            "Setting up Azure AI Search index and integrated vectorization...")
